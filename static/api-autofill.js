async function getMangaData(mangaName) {
    const res = await fetch(encodeURI(`https://kitsu.io/api/edge/manga?filter[text]=${mangaName}`));
    if(res.ok) {
        return res.json();
    } else {
...
async function getMangaCategories (mangaSlug) {
    const res = await fetch(`https://kitsu.io/api/edge/manga?fields%5Bcategories%5D=slug%2Ctitle&filter%5Bslug%5D=${encodeURI(mangaSlug)}&include=categories`);
    if(res.ok) {
        return res.json();
    } else {
...
async function autoFillItem() {
    ...
    if(searchData) {
        if(document.getElementById('rec-container')) {
            //delete element
            let toDelete = document.getElementById("rec-container");
            toDelete.remove();
        }
        ...
        for(let i = 0; i < recommendations.length; i++) {
            recContainer.appendChild(createRecCard(recommendations[i].attributes.canonicalTitle, recommendations[i].attributes.description, recommendations[i].attributes.posterImage.medium, recommendations[i].attributes.slug));
        }
